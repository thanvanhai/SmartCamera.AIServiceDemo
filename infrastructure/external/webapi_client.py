...
class WebApiClient:
    ...
    async def send_live_detection(self, detection_result: Dict[str, Any]) -> bool:
        """
        Gửi kết quả AI detection trực tiếp lên WebAPI cho real-time display
        ...
        """
        endpoint = f"{self.base_url}/api/ai/live-detection"
        ...
        try:
            async with httpx.AsyncClient(**self.client_config) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.debug(f"✅ Live detection sent successfully - Camera: {detection_result['cameraId']}")
                    return True
                else:
                    self.error_count += 1
                    logger.warning(f"⚠️ WebAPI returned {response.status_code}: {response.text}")
                    return False
                    
        except httpx.TimeoutException:
            self.error_count += 1
            logger.error(f"⏰ Timeout sending live detection - Camera: {detection_result['cameraId']}")
            return False
    ...
    async def send_alert(self, alert_data: Dict[str, Any]) -> bool:
        ...
        try:
            async with httpx.AsyncClient(**self.client_config) as client:
                ...
                if response.status_code == 200:
                    logger.info(f"🚨 Alert sent - Type: {alert_data.get('type')} - Camera: {alert_data.get('cameraId')}")
                    return True
                else:
    ...
    async def update_camera_ai_status(self, camera_id: str, status: Dict[str, Any]) -> bool:
        ...
        try:
            async with httpx.AsyncClient(**self.client_config) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"⚠️ Failed to update camera status: {response.status_code}")
                    return False
        ...
        try:
            async with httpx.AsyncClient(**self.client_config) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"⚠️ Failed to get camera config: {response.status_code}")
                    return None
        ...
        try:
            async with httpx.AsyncClient(**self.client_config) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"📦 Batch sent - {processed}/{len(detections)} detections")
                    return processed
                else:
