...
class BatchProcessor:
    ...
    async def _process_batch(self, batch: BatchJob):
        try:
            ...
            self.metrics.record_inference(
                ...
                input_size=(batch.jobs[0].frame.shape[1], batch.jobs[0].frame.shape[0]) if batch.jobs[0].frame is not None else (0,0),
    ...
    def get_statistics(self) -> Dict[str, Any]:
        return {
            ...
            'average_batch_size': (self.total_jobs_processed/self.total_batches_processed) if self.total_batches_processed>0 else 0,
